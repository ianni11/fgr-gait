  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        fontFamily: 'var(--font-mono)', fontWeight: 800, fontSize: 16, color,
      }}>
        {session.score}
      </div>
...
// ─── STORICO: vista report storico aperto ─────────────────────────────────────
function HistoricReport({ session, user, onBack }) {
  ...
  const report = {
    ...
    frames:          [],      // non salvati — mostriamo messaggio
    frameAngles:     [],
  };
    ...
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>
          ...
          {date} · {report.sampleCount} campioni · {Math.round(report.durationSeconds)}s
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 24, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '24px', marginBottom: 24, alignItems: 'center' }}>
        <ScoreRing value={report.score} size={130} />
        <div>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 16, letterSpacing: '0.06em', marginBottom: 12 }}>
            RIEPILOGO SESSIONE
          </div>
          <CriticalityBadge stats={report.stats} />
          <p style={{ marginTop: 14, color: 'var(--text-muted)', fontSize: 13, lineHeight: 1.7 }}>
            {report.score >= 80
              ? 'Ottima sessione biomeccanica. La maggior parte degli angoli rientra nei range ideali con bassa variabilità.'
              : report.score >= 60
              ? 'Sessione discreta. Alcuni angoli mostrano criticità moderate.'
      ...
      {activeTab === 'stats' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ background: 'rgba(59,130,246,0.06)', border: '1px solid rgba(59,130,246,0.15)', borderRadius: 10, padding: '14px 18px', fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.7 }}>
            La barra mostra la distribuzione degli angoli nel range di riferimento. La linea centrale è la <strong style={{color:'var(--text)'}}>media (μ)</strong>, la banda colorata è la <strong style={{color:'var(--text)'}}>deviazione standard (±σ)</strong>.
          </div>
          {ANGLE_DEFS.map(def => (
            <AngleStatRow key={def.key} def={def} stat={report.stats[def.key]} />
          ))}
        </div>
      )}

      {/* TAB: Frame — non disponibili per sessioni storiche */}
      {activeTab === 'frames' && (
        <div style={{
      ...
      {activeTab === 'timeline' && (
        <div>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '20px' }}>
            ...
            {ANGLE_DEFS.map(def => {
              const vals = report.timeline.map(s => s[def.key]).filter(v => v != null);
              if (vals.length === 0) return null;
              const stat = report.stats[def.key];
              return (
                <div key={def.key} style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ position: 'relative', height: 40, background: 'rgba(255,255,255,0.03)', borderRadius: 6, overflow: 'hidden' }}>
                    ...
                    {vals.map((deg, i) => {
                      const span = def.warn[1] - def.warn[0];
                      const xPct = report.timeline.length > 1 ? (i / (report.timeline.length - 1)) * 100 : 50;
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 3, fontSize: 9, fontFamily: 'var(--font-mono)', color: 'var(--text-faint)' }}>
                    <span>0s</span><span>{Math.round(report.durationSeconds)}s</span>
                  </div>
                </div>
              );
            })}
...
export default function SessionReport({ report, user, onNewSession }) {
  ...
  const [openSession, setOpenSession]     = useState(null); // sessione storica aperta
  ...
  // Fetch storico quando si apre il tab
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setHistoryError(data.message || 'Errore nel caricamento');
      }
    } catch {
  ...
  useEffect(() => {
    if (activeTab === 'storico' && history.length === 0 && !historyLoading) {
      fetchHistory();
    }
  }, [activeTab]);
  ...
  return (
    <div style={{ minHeight: '100vh', padding: '24px 16px 60px' }}>
      <div style={{ maxWidth: 860, margin: '0 auto' }}>
        ...
        {/* Se c'è una sessione storica aperta, mostra quella */}
        {activeTab === 'storico' && openSession ? (
          <HistoricReport
            session={openSession}
            user={user}
              ...
              <div className="fade-up">
                ...
                {historyError && (
                  <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 10, padding: '16px 18px', color: '#ef4444', fontSize: 13 }}>
                    {historyError}
                    <button onClick={fetchHistory} style={{ marginLeft: 12, background: 'none', border: 'none', color: '#ef4444', textDecoration: 'underline', cursor: 'pointer', fontSize: 13 }}>
                      Riprova
                    </button>
                  </div>
                )}
                ...
                {!historyLoading && history.length > 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', marginBottom: 4 }}>
                      {history.length} sessioni trovate · clicca per aprire il report completo
                    </div>
                    {history.map(session => (
                      <HistoryRow
                        key={session.id}
